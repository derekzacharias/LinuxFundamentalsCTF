...
| 5 | Environmental honesty follow-up: the container was masking two more lessons — `cowsay` (lesson 7's payload) was pre-installed so `apt install` was never exercised, and Ubuntu's minimized image diverts `man` to a "system has been minimized" stub even though lesson 1 builds on `man`; `getfacl`/`setfacl` (lesson 19) and `ping` (lesson 12) were taught but absent. Removed the pre-installed payload, restored man pages, added `acl`/`iputils-ping`, documented the lab's real requirements, and added toolbelt + no-pre-installed-payload invariants | 24 lessons + toolbelt + journal/game-loop/reset/CLI-stderr/solver-rerun PASS; lesson 7 now genuinely installs cowsay inside CI |
